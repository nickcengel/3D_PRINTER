...
#ifndef ZaberController_h
#define ZaberController_h
...
class ZaberController{
    public:
        enum zCommand {
            ...
            RESTORE_SETTINGS = 36,
            SET_MICROSTEP_RESOLUTION = 37,   //37 set microsteps, Reply: microsteps
        ...
        ZaberController(char deviceNum);
    
        int connectController(const char *port_name, uint16_t baudRate);
        zStatus pollStatus(zStatus endCondition);
        ...
        int set(zCommand reg, int val);
        int read(zCommand reg);
    
};

#endif /* ZaberController_hpp */
